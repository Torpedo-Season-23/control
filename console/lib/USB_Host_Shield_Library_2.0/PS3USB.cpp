...
PS3USB::PS3USB(USB *p, uint8_t btadr5, uint8_t btadr4, uint8_t btadr3, uint8_t btadr2, uint8_t btadr1, uint8_t btadr0) : pUsb(p),            // pointer to USB class instance - mandatory
...
bool PS3USB::getButtonPress(ButtonEnum b) {
  const int8_t index = getButtonIndexPS3(b);
  if (index < 0) return 0;
...
uint8_t PS3USB::getAnalogButton(ButtonEnum a) {
  const int8_t index = getButtonIndexPS3(a);
  if (index < 0) return 0;
