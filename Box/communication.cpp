...
void CommunicationClient::sendData(uint16_t *imu_frame,int imu_size,uint8_t current,int current_size,uint16_t pressure) {
  ...
  udp.write(data_sent,10);
  udp.endPacket();
  udp.stop();
}
void CommunicationClient::prep_to_send(uint16_t *imu_frame, int imu_size, uint8_t current, int current_size, uint16_t pressure) {
  int x = 0;
